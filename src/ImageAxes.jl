...
_channelview(A::AxisArray{C,N}, Ac::AbstractArray{T,N}) where {C,T,N} = AxisArray(Ac, axes(A)...)
# with extra dimension: (bug: the type parameters shouldn't be necessary, but julia 0.5 dispatches incorrectly without them)
_channelview(A::AxisArray{C,M}, Ac::AbstractArray{T,N}) where {C,T,M,N} = AxisArray(Ac, Axis{:color}(indices(Ac,1)), axes(A)...)
...
"""
timedim(img::AxisArray{T,N}) where {T,N} = _timedim(filter_time_axis(axes(img), ntuple(identity, Val{N})))
...
ImageCore.coords_spatial(img::AxisArray{T,N}) where {T,N} = filter_space_axes(axes(img), ntuple(identity, Val{N}))
